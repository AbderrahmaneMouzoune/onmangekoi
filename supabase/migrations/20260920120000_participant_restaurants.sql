...
--   * `create_session` et `export_my_data` sont reprises telles que
--     `timed_sessions` les a laissées, `closes_at` compris : les redéfinir
--     sans repartir de la dernière version reviendrait à annuler la sienne.
-- ============================================================
...
-- Reprise à l'identique de `timed_sessions` (signature à trois paramètres
-- comprise), au `added_by` près : les restos posés à la création sont ceux du
-- host, et doivent le dire.
...
-- export. Le corps est celui de `timed_sessions` (`closes_at` compris), à deux
-- clés près :
--   * `restaurants_added` dans chaque participation, la nouveauté de cette
--     migration ;
--   * `contributed_restaurants`, rétabli — `manual_restaurants` l'avait ajouté
--     pour `restaurants.created_by`, et la réécriture de `timed_sessions` l'a
--     perdu au passage. La colonne existe toujours, la donnée reste celle de
--     la personne : elle doit figurer dans son export.
