...
--   * une session datée (`timed_sessions`) se complète comme une autre tant
...
-- ─── UNE ÉCHÉANCE NE FERME PAS LA SALLE D'ATTENTE ────────────
-- `closes_at` est posé dès la création, mais ne pèse qu'au lancement : d'ici
-- là, la session se complète comme n'importe quelle autre.
select pg_temp.act_as(:'alice');

select id as timed_id, invite_code as timed_code
from public.create_session('Dîner minuté', array[:'resto1'::uuid], now() + interval '30 minutes')
\gset

select pg_temp.act_as(:'bob');
select public.join_session(:'timed_code');
select public.add_session_restaurant(:'timed_id'::uuid, :'resto2'::uuid);

select pg_temp.assert(
  (
    select count(*) from public.session_restaurants where session_id = :'timed_id'::uuid
  ) = 2,
  'une session datée accepte encore un resto avant son lancement'
);

select pg_temp.assert(
  exists (
    select 1 from public.sessions
    where id = :'timed_id'::uuid and closes_at is not null and status = 'waiting'
  ),
  'l’échéance de la session datée est intacte'
);
