...
import { codeFromSlug } from '@/lib/slug'
...
function parseRouteParam(param: string, length: number): RouteIdentifier {
  ...
  const code = codeFromSlug(raw, length)
  return code ? { kind: 'code', value: code } : { kind: 'invalid', value: raw }
}

/** `/sessions/dej-du-lundi-7K3M9P` → code ; un ancien `/sessions/<uuid>` reste lu. */
export function parseSessionParam(param: string): RouteIdentifier {
  return parseRouteParam(param, INVITE_CODE_LENGTH)
}

/** `/lists/restos-du-bureau-7K3M9P2QWX` → code ; un ancien `/lists/<uuid>` reste lu. */
...
export function parseSharedListParam(param: string): ShareIdentifier {
  ...
  const code = codeFromSlug(raw, SHARE_CODE_LENGTH)
  return code ? { kind: 'code', value: code } : { kind: 'invalid', value: raw }
}

/** Un identifiant d'invitation de session : code court, slug lisible, token long ou lien collé. */
export function parseInviteIdentifier(raw: string): ShareIdentifier {
  ...
  const code = codeFromSlug(value, INVITE_CODE_LENGTH)
