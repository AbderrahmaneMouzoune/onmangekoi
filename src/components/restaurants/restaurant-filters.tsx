...
import { RiCloseLine, RiMapPin2Line } from '@remixicon/react'
...
interface RestaurantFiltersBarProps {
  ...
  /** Position du navigateur : sans elle, le filtre distance n'existe pas. */
  geolocation: GeolocationState
}

/**
 * Chips de filtre au-dessus du sélecteur : budget, régime, distance.
 *
 * Tout est filtré en base — l'interface ne fait que dire ce qu'elle demande.
 * Le filtre distance n'apparaît que si la géolocalisation est possible : pas
 * de bouton mort quand le navigateur ne sait pas faire ou que la personne a
 * déjà refusé.
 */
export function RestaurantFiltersBar({ value, onChange, geolocation }: RestaurantFiltersBarProps) {
  const activeCount = countActiveFilters(value)
  const canLocate = geolocation.status !== 'unsupported' && geolocation.status !== 'denied'
  const isLocated = geolocation.status === 'granted' && geolocation.position !== null
    ...
    <div className="flex flex-col gap-3 rounded-lg bg-surface p-3 ring-1 ring-line">
        ...
        <fieldset className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
          ...
          {isLocated ? (
            ...
            >
              {DISTANCE_CHOICES_KM.map((km) => (
                <Chip
                  ...
                  label={`Moins de ${distanceLabel(km)} d’ici`}
            ...
            <button
              ...
              disabled={geolocation.status === 'pending'}
              className="inline-flex h-8 items-center gap-1.5 rounded-full border border-line-strong bg-surface px-3 text-xs font-semibold text-ink-2 transition-colors hover:bg-surface-2 disabled:opacity-70"
            >
      ...
      {activeCount > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs text-muted-foreground">{hiddenNotice(value)}</p>
          <button
            type="button"
 ...
 * à un catalogue plus pauvre qu'il n'est.
 */
